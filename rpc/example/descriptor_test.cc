...
int main()
{
  ...
  testDescriptor<miren::Query>();
  testDescriptor<miren::Answer>();

  google::protobuf::Message* newQuery = createMessage("miren.Query");
  assert(newQuery != NULL);
  assert(typeid(*newQuery) == typeid(miren::Query::default_instance()));
  cout << "createMessage(\"miren.Query\") = " << newQuery << endl;

  google::protobuf::Message* newAnswer = createMessage("miren.Answer");
  assert(newAnswer != NULL);
  assert(typeid(*newAnswer) == typeid(miren::Answer::default_instance()));
  cout << "createMessage(\"miren.Answer\") = " << newAnswer << endl;
