...
//http请求
class HttpRequest : public Miren::base::Copyable
{
 public:
  enum Method   //请求方法
  {
  ...
  bool setMethod(const char* start, const char* end)  //设置方法
  {
    ...
    std::string m(start, end);  //使用字符串首尾构造string，不包括尾部，如char *s="123", std::string s=(s,s+3),则s输出为123
    if (m == "GET")
    {
      method_ = kGet;
    }
  ...
  void setReceiveTime(base::Timestamp t)  //设置接收时间
  { receiveTime_ = t; }

  base::Timestamp receiveTime() const
  { return receiveTime_; }

//添加头部信息，客户传来一个字符串，我们把它转化成field: value的形式(头部字段表示：字段名称 冒号 空格 字段的值)
  void addHeader(const char* start, const char* colon, const char* end)
  {
    std::string field(start, colon);  //得到字段名称
    ++colon;
    while (colon < end && isspace(*colon))  //跳过空格
    {
      ++colon;
    }
    std::string value(colon, end);  //获取字段的值
    while (!value.empty() && isspace(value[value.size()-1]))  //去除右空格，如果右边有空格会一直resize-1
    {
      value.resize(value.size()-1);
    }
    headers_[field] = value;  //把对应字段对应值存储起来(std::map<std::string, std::string> headers_)
  }

  std::string getHeader(const std::string& field) const  //根据头部字段返回值内容
  {
    std::string result;
  ...
  const std::map<std::string, std::string>& headers() const    //返回头部列表
  { return headers_; }
 ...
 private:
  ...
  base::Timestamp receiveTime_; //请求时间
  std::map<std::string, std::string> headers_;  //头部列表,存储 字段对应的值
};
