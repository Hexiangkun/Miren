#include "example/http/HttpServer.h"

#include "base/log/Logging.h"
#include "example/http/HttpContext.h"
#include "example/http/HttpRequest.h"
#include "example/http/HttpResponse.h"
    ...
    httpCallback_(detail::defaultHttpCallback)
{
  server_.setConnectionCallback(
      std::bind(&HttpServer::onConnection, this, std::placeholders::_1));  //连接到来回调该函数
  server_.setMessageCallback(
      std::bind(&HttpServer::onMessage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));  //消息到来回调该函数
}
...
{
  HttpContext* context = std::any_cast<HttpContext>(conn->getMutableContext("httpContext"));  //取出请求，mutable可以改变
...
{
  const std::string& connection = req.getHeader("Connection"); //取出头部Connection对应的内容
