#include "http/parser/base/Address.h"
#include "http/parser/base/Endian.h"
#include <sstream>
#include <netdb.h>
#include <ifaddrs.h>
#include <stddef.h>
...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
    ...
    if(error) {
        std::cerr << "Address::Lookup getaddress(" << host << ", "
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
    ...
    if(result <= 0) {
        std::cerr << "IPv6Address::Create(" << address << ", "
                << port << ") rt=" << result << " errno=" << errno
                << " errstr=" << strerror(errno);
        return nullptr;
    }
...
UnixAddress::UnixAddress(const std::string& path) {
    memset(&m_addr, 0, sizeof(m_addr));
    m_addr.sun_family = AF_UNIX;
    m_length = static_cast<socklen_t>(path.size() + 1);
...
UnknownAddress::UnknownAddress(sa_family_t family) {
    memset(&m_addr, 0, sizeof(m_addr));
